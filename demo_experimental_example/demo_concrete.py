"""
This script runs the concrete constitutive model example for the paper:
...
test_filename = os.path.join(open_dir, f'concrete_clean.mat')
test_data = loadmat(test_filename)
test_set = test_data['concrete_clean']  
strain_test = test_set[:, 0]
stress_test = test_set[:, 1]

for j in range(1, n_simulations+1):
    # Load saved parameter dictionary
    with open(os.path.join(open_dir, f'RROKL_Architecture_Concrete{j}_all.pkl'), 'rb') as f:
    ...
    for i, ratio in enumerate(ratios):
        ...
        opt_gamma = params['gamma'].item()*1     
        opt_sigma2 = params['sigma2'].item()            
        opt_Xsub = params['Xsub']       

        # 2. Load training data
        train_filename = os.path.join(open_dir, f'concrete_outlier_set{j}-{int(ratio*100)}.mat')
        train_data = loadmat(train_filename)
        train_set = train_data['concrete_outlier']  
...
# Save results
np.savez('results_concrete.npz', R2=R2, NRMSE=NRMSE)