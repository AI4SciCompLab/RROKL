"""
This script runs the steel constitutive model example for the paper:
...
test_filename = os.path.join(open_dir, f'steel_clean.mat')
test_data = loadmat(test_filename)
test_set = test_data['steel_clean'] 
strain_test = test_set[:, 0]
stress_test = test_set[:, 1]

for j in range(1, n_simulations+1):
    # Load saved parameter dictionary
    with open(os.path.join(open_dir, f'RROKL_Architecture_Steel{j}_all.pkl'), 'rb') as f:
    ...
    for i, ratio in enumerate(ratios):
        ...
        train_filename = os.path.join(open_dir, f'steel_outlier_set{j}-{int(ratio*100)}.mat')
        train_data = loadmat(train_filename)
        train_set = train_data['steel_outlier']  
...
np.savez('results_steel.npz', R2=R2, NRMSE=NRMSE)